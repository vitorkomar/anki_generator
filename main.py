...
def main():
    ...
    pairs = get_sentences_with_word(args.word, args.from_lang, args.to_lang, args.number_of_words)
    translation = get_word_translation(args.word, args.from_lang, args.to_lang)

    word_with_article = args.word
    if args.from_lang == "de":
        article = get_german_word_article(args.word)
        if article:
    ...
    reversed_note = generate_reverse_card(args.word, translation, pairs, audio=True)
    ...
    pkg.media_files = [word_with_article + ".mp3" ]+ [
